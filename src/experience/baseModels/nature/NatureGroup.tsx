...
export function Instances({ children, ...props }: InstancesProps) {
  const { nodes } = useGLTF('/models/nature_group.glb') as unknown as GLTFResult;
  ...
  return (
    <Merged meshes={instanceMeshes} {...props}>
      {meshes => (
        <context.Provider value={meshes as unknown as NatureInstances}>{children}</context.Provider>
      )}
    </Merged>
  );
}
